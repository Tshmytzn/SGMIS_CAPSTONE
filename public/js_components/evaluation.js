...
function DisplayAddForm() {
        ...
        if (res.eval.length === 0) {
          list.innerHTML = `<div class="empty" id="empty_eval">
           ...
           <p class="empty-subtitle text-muted">
             ${where != 'student' ? 'Try adding one to let students evaluate the event.' : 'No Event is currently being evaluated'}
           </p>
           ${where != 'student' ? `<div class="empty-action">
      ...
      $.ajax({
          ...
          success: res=>{
              ...
              res.data.forEach(d => {
                ...
                container.classList.add('container', 'mt-4', 'border', 'p-4', 'rounded');

                // Create the content for each question dynamically
                const questionContent = `<p class="fw-bold">#${numCount} ${d.eq_question}(${scaleType[0]})</p>`;
                ...
                evaluationResult.appendChild(container);

                switch (scaleType[1]) {
                    case 'bar':
                        loadBarChart(getRate(d.eq_scale), d.eval_data, chartId);
                        break;
                    case 'pie':
                        loadPieChart(d.eval_data, chartId);
                        break;
                    default:
                        loadDataTable(chartId, d.eval_data);
                        break;
                }
  ...
  function loadPieChart(data, id){
      window.ApexCharts && (new ApexCharts(document.getElementById(id), {
          ...
          colors: [tabler.getColor("primary"), tabler.getColor("primary", 0.6)],
          legend: {
              show: true,
              position: 'bottom',
              offsetY: 12,
  ...
  function loadDataTable(id, data){
    $(`#${id}`).DataTable( {
        data:data,
        columns: [
            {title: "Student ID", data: 'school_id'},
            {title: "Answer", data: "res_value"}
        ]
    });
  }
