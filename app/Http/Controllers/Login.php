...
class Login extends Controller
{
   public function AdminLogin(Request $req){
    ...
    $student = StudentAccounts::where('school_id',$req->username)->where('student_type','Administrator')->first();
    if($admin){
      ...
      if (Hash::check($req->password, $student->student_pass)) {
        Session::put('admin_id', $student->student_id);
        return response()->json(['status' => 'success']);
      } else {
        return response()->json(['status' => 'incorrect']);
      }
